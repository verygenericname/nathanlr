...
int preserve_copyfile(const char* machoPath, const char* outputPath) {
    struct stat st;
    stat(outputPath, &st);
    mode_t original_mode = st.st_mode;

    int r = copyfile(machoPath, outputPath, 0, COPYFILE_ALL | COPYFILE_MOVE | COPYFILE_UNLINK);

    chmod(outputPath, original_mode);
...
int apply_coretrust_bypass_wrapper(const char *inputPath, const char *outputPath, char *teamID, char *appStoreBinary)
{
    ...
    r = preserve_copyfile(machoPath, outputPath);
    if (r == 0) {
