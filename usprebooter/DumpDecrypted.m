...
//			[self makeDirectories:encryptedImageFilenameStr];
            ...
            size_t original_len = strlen(encryptedImageFilenameStr);
            size_t suffix_len = strlen("_DECRYPTED");
            size_t buffer_size = original_len + suffix_len + 1;
            char result2[buffer_size];
            snprintf(result2, sizeof(result2), "%s%s", encryptedImageFilenameStr, "_DECRYPTED");
            
			NSLog(@"[dumpDecrypted] Dumping: %s", encryptedImageFilenameStr);
			NSLog(@"[dumpDecrypted]    Into: %s", result2);
			...
			outfd = open(result2, O_RDWR|O_CREAT|O_TRUNC, 0644);
			if (outfd == -1) {
				NSLog(@"[dumpDecrypted] Failed opening: %s", result2);
				return false;
			}
...
-(void) dumpDecrypted:(pid_t)pid fileName:(const char *)fileNameStr {
